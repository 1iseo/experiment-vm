...
export type SsaId = string; // "v_0x1a4" (contoh id SSA)
export type BlockId = string; // b_0x00 (contoh id blok)
export type VmPtr = number; // Alamat/slot memori virtual
...
export interface BaseInst {
  id: SsaId;
  deps: Dependency[]; // Graf dependensi eksplisit yang independen dari argumen
...
export interface PhiChoice {
  block: BlockId | "ANY"; // 'ANY' memutus kebergantungan pada dominasi CFG
  val: SsaId;
}
...
export interface IRFunction {
  ...
  entry: BlockId;
  memLayout: Record<string, VmPtr>; // Memetakan variabel asli ke tata letak memori VM
  globals?: Record<string, VmPtr>; // Opsional: pemetaan lingkungan global
  nestedFunctions?: IRFunction[]; // Literal fungsi bersarang (getter/setter, dll.)
  captures?: string[]; // Nama yang ditangkap untuk pengaturan entri closure
