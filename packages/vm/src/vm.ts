  ...
  while (state.pc < code.length) {
    try {
      while (state.pc < code.length) {
        ...
        switch (opcode) {
          ...
          case Opcode.DISPATCH: {
            // Enkoding: u8 jumlah kasus, lalu pasangan (u8 indeksPool, u16 offset), lalu u16 default.
          ...
          case Opcode.DISPATCH_W: {
            // Enkoding: u8 jumlah kasus, lalu pasangan (u16 indeksPool, u32 offset), lalu u32 default.
            const discr = stack.pop();
            const caseCount = readU8();
            let target = -1;
            for (let i = 0; i < caseCount; i += 1) {
      ...
      while (true) {
        ...
        state.args = frame.savedArgs;
        state.contextThis = frame.savedContextThis;
        // Juga perbarui referensi state untuk memory dan stack agar sesuai dengan frame yang dipop
        state.memory = memory;
        state.stack = stack;
      }
      if (!handled) {
        // Bersihkan call stack dan lemparkan error
