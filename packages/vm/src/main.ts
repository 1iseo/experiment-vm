...
function compileToVM(ast: t.Node): VM_Instruction[] {
  ...
  function compile(node: t.Node | null | undefined) {
    ...
    if (t.isBinaryExpression(node)) {
      // kompilasi kiri lalu kanan, kemudian keluarkan operator
    ...
    // Untuk sekarang, abaikan jenis node lain (identifier, panggilan fungsi, dll.)
  }
...
function execute(vm: VM) {
  while (vm.pc < vm.code.length) {
    ...
    switch (instr.type) {
      ...
      // Tangani instruksi lain (SUB, MUL, DIV) secara serupa
    }
    vm.pc++;
  }
}
...
function prettyPrintSSA(fn: SSAFunction) {
  ...
  for (const block of fn.blocks) {
    ...
    for (const instr of block.instructions) {
      ...
      lines.push(
        `  ${fmtValue(id)} = ${instr.op} ${instr.args.map(fmtValue).join(", ")}`,
      );
    }
  }
