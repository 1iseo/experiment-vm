...
function allocateSsaMemory(fn: IRFunction): {
  map: Record<SsaId, VmPtr>;
  size: number;
} {
  ...
  for (const block of blocks.values()) {
    ...
    for (const inst of block.insts) {
      ...
      for (const choice of inst.choices) {
        ...
        if (!origin) {
          throw new Error(
            `Phi lowering failed: block '${originId}' not found.`,
          );
        }
...
export function lowerIRFunctionToBytecode(
  fn: IRFunction,
  ...
  const otherBlocks = Array.from(blocks.values()).filter(
    (block) => block.id !== fn.entry,
  );
    ...
    for (const inst of block.insts) {
      ...
      switch (inst.type) {
        ...
        case "Call": {
          ...
          emitPushConstValue(
            inst.directEval ? inst.args.length + 1 : inst.args.length,
          );
          emitOpcode(
            inst.directEval
              ? Opcode.CALL_EVAL
        ...
        default: {
          ...
          throw new Error(
            `Unsupported instruction: ${(neverInst as Instruction).type}`,
          );
        }
      }
  ...
  for (let i = 0; i < orderedBlocks.length; i += 1) {
    ...
    if (errorPtr === undefined) {
      throw new Error(
        `Missing catch parameter pointer for handler '${block.catchTarget}'.`,
      );
    }
    ...
    if (
      last &&
      last.handlerPc === handlerPc &&
      last.errorPtr === errorPtr &&
      last.endPc === startPc
    ) {
      last.endPc = endPc;
    } else {
...
export function lowerIRProgramToBytecode(
  program: IRProgram,
  options?: LoweringOptions,
  ...
  const functionMeta = new Map<
    string,
    { map: Record<SsaId, VmPtr>; size: number }
  >();
  ...
  const backpatches: Array<{
    offset: number;
    funcId: string;
    target: BlockId;
    width: 2 | 4;
  }> = [];
  const closureBackpatches: Array<{
    offset: number;
    targetFunc: string;
    width: 2 | 4;
  }> = [];
  ...
  const rawExceptionEntries: Array<{
    startPc: number;
    endPc: number;
    funcId: string;
    catchTarget: BlockId;
  ...
  for (const [funcId, fn] of program.functions.entries()) {
      ...
      for (const inst of block.insts) {
        ...
        switch (inst.type) {
          ...
          case "BinOp": {
            emitLoadSsa(inst.left);
            emitLoadSsa(inst.right);
            emitOpcode(binOpMap[inst.op]!);
            emitStoreSsa(inst.id);
            break;
          }
          case "UnOp": {
            emitLoadSsa(inst.arg);
            emitOpcode(unOpMap[inst.op]!);
            emitStoreSsa(inst.id);
            break;
          }
          case "Move": {
          ...
          case "AllocClosure": {
            ...
            closureBackpatches.push({
              offset: bytecode.length,
              targetFunc: inst.targetFunc,
              width: 4,
            });
            emitU32(0xffffffff); // placeholder entryPc

            const targetMeta = functionMeta.get(inst.targetFunc);
            if (!targetMeta)
              throw new Error(`Unknown targetFunc '${inst.targetFunc}'`);
            if (targetMeta.size > 0xffffffff)
            ...
            if (selfRefPtr > 0xffffffff)
              throw new Error("Self-reference pointer overflow");
            emitU32(selfRefPtr);
          ...
          default: {
            throw new Error(
              `Unsupported instruction: ${(inst as Instruction).type}`,
            );
          }
        }
        debugMap?.instructionRanges.push({
    ...
    for (let i = 0; i < orderedBlocks.length; i += 1) {
      ...
      if (
        ...
        throw new Error(
          `Exception table range missing for block '${block.id}'.`,
        );
      }

      rawExceptionEntries.push({
        startPc,
        endPc,
        funcId,
  ...
  for (const raw of rawExceptionEntries) {
    const handlerPc = globalBlockOffsets.get(
      blockKey(raw.funcId, raw.catchTarget),
    );
    if (handlerPc === undefined)
      throw new Error(`Handler pc missing for ${raw.catchTarget}`);

    const last = exceptionTable[exceptionTable.length - 1];
    if (
      last &&
      last.handlerPc === handlerPc &&
      last.errorPtr === raw.errorPtr &&
      last.endPc === raw.startPc
    ) {
  ...
  for (const patch of backpatches) {
    const targetOffset = globalBlockOffsets.get(
      blockKey(patch.funcId, patch.target),
    );
    if (targetOffset === undefined)
      throw new Error(`Backpatch target '${patch.target}' missing.`);
    if (patch.width === 2) {
  ...
  for (const patch of closureBackpatches) {
    ...
    if (targetOffset === undefined)
      throw new Error(`Closure target '${patch.targetFunc}' missing.`);
    if (patch.width === 2) {
      if (targetOffset > 0xffff)
        throw new Error(`Closure offset overflow: ${targetOffset}`);
      bytecode[patch.offset] = (targetOffset >> 8) & 0xff;
      bytecode[patch.offset + 1] = targetOffset & 0xff;
    } else {
      bytecode[patch.offset] = Math.floor(targetOffset / 0x1000000) & 0xff;
