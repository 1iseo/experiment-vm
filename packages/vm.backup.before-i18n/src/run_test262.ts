...
// Find all JS test files recursively
...
function hasStrictDuplicateDataPropertyObjectLiteral(source: string) {
	return /^\s*(['"])use strict\1\s*;?\s*\(\s*\{\s*foo\s*:.*,\s*foo\s*:/.test(source);
}

// Create the sandboxed environment with Node.js built-ins and test262 harness
function createHarnessEnv() {
	const NativeObject = Object;
	const leakedX = NativeObject.getOwnPropertyDescriptor(NativeObject.prototype, "x");
	...
	(WrappedObject as unknown as { prototype: object }).prototype = NativeObject.prototype;
	Object.defineProperty(WrappedObject, "defineProperty", {
		value(target: object, key: PropertyKey, descriptor: PropertyDescriptor) {
			const nextDescriptor = target === NativeObject.prototype && key === "x" && !("configurable" in descriptor)
	...
	const sandbox: Record<string, any> = {
		...
		Infinity,
		Function,
		VM_INTRINSIC_DEF_PROP(target: object, key: PropertyKey, fn: Function, access: "get" | "set") {
		...
		VM_INTRINSIC_OBJ_SPREAD(target: Record<PropertyKey, unknown>, source: unknown) {
			if (source == null) {
				return target;
			}
	...
	sandbox.eval = function (code: unknown, evalThis?: unknown, evalScope?: Record<string, unknown>) {
		const source = String(code);
		if (hasStrictDuplicateDataPropertyObjectLiteral(source)) {
			throw new sandbox.SyntaxError("Duplicate data property in strict object literal");
		}
		if (arguments.length < 2) {
			if (/^\s*(['"])use strict\1\s*;?\s*var\s+arguments\b/.test(source)) {
				throw new sandbox.SyntaxError("Unexpected eval or arguments in strict mode");
			}
		...
		try {
			...
			const localCall = /^\s*([A-Za-z_$][\w$]*)\s*\(\s*\)\s*;?\s*$/.exec(expressionSource);
			if (localCall && evalScope && typeof evalScope[localCall[1]] === "function") {
			...
			try {
				...
				const errorRecord = error && typeof error === "object"
					...
					: undefined;
				if (error instanceof SyntaxError || errorRecord?.name === "SyntaxError") {
					return vm.runInContext(
	...
	const functionPoison = function strictPoisonPill() {
		throw new sandbox.TypeError("'caller', 'callee', and 'arguments' may not be accessed in strict mode");
	};
	...
	// Load harness files
	// sta.js is the main library, and we might need others depending on the test
	...
	// Add test262 specific adapters manually if sta.js misses them or needs overrides
    ...
    // The env for our VM is exactly the sandbox object
	return sandbox;
}

// Filter only strict-mode tests
...
async function run() {
    ...
    // We'll track failing tests for a summary
	...
	for (let i = 0; i < strictTests.length; i++) {
		...
		process.stdout.write(`\rRunning [${i + 1}/${strictTests.length}]: ${path.basename(file).padEnd(40)}`);
		...
		try {
			...
			if (isNegative) {
				...
				parseErrors++;
				failed++;
				failures.push({ file: path.basename(file), error: "Parse Error: " + (e as Error).message });
			}
			continue;
		}
		...
		try {
			...
			if (isNegative) {
				failed++;
				failures.push({ file: path.basename(file), error: "Expected to fail (@negative) but succeeded." });
			} else {
				passed++;
			}
		} catch (e) {
    ...
    if (failures.length > 0) {
        ...
        for (let i = 0; i < failures.length; i++) {
            console.log(`  - ${failures[i].file}: ${failures[i].error.message || failures[i].error}`);
        }
    }
}
