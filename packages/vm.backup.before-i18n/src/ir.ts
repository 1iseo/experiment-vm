...
type IRValue = {
    ...
    // Dependencies (edges in SSA graph)
    deps: Dependency[];

    // Who uses this value
    uses: InstrId[];

    // Type is optional (JS is dynamic)
    type?: IRType;
...
type IRPhi = BaseInstr & {
    ...
    // NOTE: not strictly tied to predecessors
    inputs: ValueId[];

    // Optional mapping (can be fake!)
    fromBlocks?: BlockId[];
};
...
export function transformFunctionToIR(node: t.Function): IRFunction {
    ...
    // Environment: variable -> latest SSA value
    ...
    function addDep(target: ValueId, dep: ValueId, kind: "real" | "fake" = "real") {
    ...
    if (!terminated) {
        // Optional: return last assigned value
...
function formatInstr(instr: IRInstruction): string {
    const outputPrefix = instr.outputs.length > 0 ? `${instr.outputs.join(", ")} = ` : "";

    switch (instr.kind) {
        case "const":
            return `${outputPrefix}const ${formatLiteral(instr.value)}`;
        case "binop":
            return `${outputPrefix}${instr.kind} ${instr.op} ${instr.left}, ${instr.right}`;
        case "unop":
            return `${outputPrefix}${instr.kind} ${instr.op} ${instr.operand}`;
        case "phi":
            return `${outputPrefix}phi [${instr.inputs.join(", ")}]${
                instr.fromBlocks ? ` from [${instr.fromBlocks.join(", ")}]` : ""
            }`;
        case "load":
            return `${outputPrefix}load ${instr.address}`;
        case "store":
            return `store ${instr.address}, ${instr.value}`;
        case "call":
            return `${outputPrefix}call ${instr.callee}(${instr.args.join(", ")})`;
        case "move":
            return `${outputPrefix}move ${instr.from}`;
        case "nop":
            return `${outputPrefix}nop`;
    }
}
...
function formatTerminator(term: IRTerminator): string {
    switch (term.kind) {
        ...
        case "return":
            return term.value ? `return ${term.value}` : "return";
        case "switch":
            return `switch ${term.discriminant} { ${term.cases
                .map((caseEntry) => `${formatLiteral(caseEntry.value)} -> ${caseEntry.target}`)
                .join(", ")} default -> ${term.default} }`;
    }
}
...
export function prettyPrintIRFunction(fn: IRFunction): string {
    ...
    lines.push(`function ${fn.id} {`);
    lines.push(`  entry: ${fn.entry}`);

    if (fn.meta && Object.keys(fn.meta).length > 0) {
        lines.push(`  meta: ${JSON.stringify(fn.meta)}`);
    }
    ...
    for (const block of fn.blocks.values()) {
        ...
        const blockMeta = formatBlockMeta(block.meta);
        if (blockMeta) {
            lines.push(`      meta: ${blockMeta}`);
        }

        if (block.predecessors.length > 0) {
            lines.push(`      predecessors: ${block.predecessors.join(", ")}`);
        }

        if (block.successors.length > 0) {
            lines.push(`      successors: ${block.successors.join(", ")}`);
        }

        for (const instrId of block.instructions) {
            const instr = fn.instructions.get(instrId);
            if (!instr) {
                lines.push(`      ${instrId}: <missing instruction>`);
                continue;
            }

            lines.push(`      ${instr.id}: ${formatInstr(instr)}`);
        }

        lines.push(`      terminator: ${formatTerminator(block.terminator)}`);
    }
